 ...
 *
 * It was `Pick<…, 'byType' | 'byAlias'>` while those two lookups were all this file called, which
 * documented its own appetite and let a test inject a hand-written stub. `UmbExtensionsManifestInitializer`
 * ends that: it takes an `UmbExtensionRegistry`, that class declares `#private` fields, and a class
 * with private fields is nominal in TypeScript, so *no* structural subset of it is assignable no
 * matter how many methods the subset lists. The choice was therefore between widening here and
 * casting at the one call site, and a cast would be a lie told in the place where it is least
 * visible: the initializer really does reach past `byType`/`byAlias` into `byTypeAndAliases` (it
 * observes each app's condition manifests) and would reach further still if it were given an array
 * of types or a filter.
 *
 ...
 *
 * The second is not about timing at all, and it came with the registered apps: a `umbraDesktopApp`
 * manifest may carry `conditions`, and a condition can flip while the desktop is up. Nothing else
 * here does that. A `ref` that has registered stays registered, and a user's permitted sections do
 * not change under them mid-session, so for every other input "observe" is only insurance against
 * arriving late. For a conditioned app there is no moment at which sampling would have been correct:
 * the answer is a function of state the desktop does not own (a workspace, a variant, a user
 * permission) and is *expected* to change. Which is why the app list has to be able to shrink as
 * well as grow, and why `#recompute` rebuilds it rather than accumulating into it.
 */
