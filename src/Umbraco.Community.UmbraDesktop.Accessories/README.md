...
- **Paint.** Pencil, brush, eraser and fill, in MS Paint's own twenty-eight colours: left click paints the foreground and right click the background. Undo with Ctrl+Z. Drag the handles on the picture's right edge, bottom edge or corner to resize it: it grows into the background colour and crops when made smaller. Works on a new picture or on an image opened from the media library.
...
- **Clock.** An analogue face with the time and the date under it, written the same way as the taskbar clock: the desktop's Regional format and Clock settings apply to both.
