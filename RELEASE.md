...
| `Umbraco.Community.UmbraDesktop.Entertainment` | Optional games add-on. Minesweeper and Snake today. |
...
| `entertainment-games-snake.png` | Snake's shot, framed to match Minesweeper's: Windows 98, the Games group open in the launcher. Also in both readmes. Take it **before** the first key press, so the start message is showing and the snake is sitting in the middle row above it, since that is the one moment the board says how to play. |
